...
async function testDSAPlayground() {
  ...
  for (const route of routes) {
    try {
      ...
      // Check for console errors on this route
      if (errors.length > 0) {
        console.log(`Console errors on ${route}:`, errors);
        results.failed.push({ route, reason: 'Console errors', errors: [...errors] });
        errors.length = 0;
      } else {
        console.log(`No console errors on ${route}`);
        results.passed.push({ route, check: 'No console errors' });
      }
  ...
  // Save results
  fs.writeFileSync('dsa-results-fixed.json', JSON.stringify(results, null, 2));
  console.log('\n=== DSA PLAYGROUND RESULTS (FIXED) ===');
